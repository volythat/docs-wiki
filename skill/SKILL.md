...
| "cập nhật mục lục" / "update TOC" | Regenerate `README.md` by SCANNING `<docs_dir>` (not a hardcoded list → new files appear automatically) + an endpoint list from `.bru` files in `<dirs.bruno>`. Use `project.name` for the title. |
