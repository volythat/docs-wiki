...
| "phân tích docs cũ từ [folder]" | **Kiểm tra trước:** nếu `<docs_dir>` trùng với `[folder]` → DỪNG, yêu cầu đặt `docs_dir` khác trong `.docswiki.yml` trước (ví dụ `docs-v2`). `[folder]` chỉ được ĐỌC, không bao giờ bị sửa hay ghi đè. **Pha 1 — Phân tích:** Đọc toàn bộ file trong `[folder]`, phân loại từng đoạn nội dung thành: (A) *Định nghĩa* (thuật ngữ, entity, field, flow) → đề xuất đưa vào file nào trong `<dirs.sources>/`; (B) *Tổng hợp/góc nhìn* → đề xuất thành file dẫn xuất nào; (C) *Endpoint/API* → đề xuất đưa vào `<dirs.api>/api.html`; (D) *Không rõ* → liệt kê riêng để người dùng quyết định. Xuất báo cáo phân loại, hỏi xác nhận hoặc điều chỉnh. **Pha 2 — Thực thi:** Chỉ sau khi người dùng xác nhận, tạo lần lượt từng file vào `<docs_dir>` — nguồn trước, dẫn xuất sau. KHÔNG đụng vào `[folder]` ở bất kỳ bước nào. |
