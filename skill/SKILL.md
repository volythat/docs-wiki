...
| "tái cấu trúc docs" | Sửa lại `<docs_dir>` hiện tại thành đúng chuẩn docs-wiki — KHÔNG xóa làm lại từ đầu. **Pha 1 — Kiểm tra:** Đọc toàn bộ file trong `<docs_dir>`, xác định: (A) nội dung định nghĩa đang nằm inline trong file dẫn xuất → cần chuyển vào `<dirs.sources>/`; (B) file chưa được nhóm vào subfolder theo domain; (C) file đã đúng chuẩn (chỉ link). Xuất báo cáo "cần sửa gì ở đâu" + cây thư mục đề xuất sau khi tái cấu trúc. Hỏi xác nhận. **Pha 2 — Thực thi (từng bước, không phá vỡ liên kết):** (1) Trích xuất định nghĩa từ file dẫn xuất → ghi vào `<dirs.sources>/` với anchor đúng; (2) Thay nội dung vừa trích xuất bằng link tới anchor tương ứng; (3) Di chuyển file vào subfolder đề xuất, cập nhật toàn bộ link trỏ vào file đó; (4) Báo từng thay đổi đã thực hiện. Không xóa file nào mà không có xác nhận riêng. |
